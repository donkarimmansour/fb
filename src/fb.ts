...
import { autoScroll, generateFacebookGroupURLById, getOldPublications,} from './common/fbHelpers';
...
export default class Facebook {
 ...
 /**
  ...
  public async enterAuthCode(
    ...
    let c1 : any = await this.page.$x(authCodeContinueButtonSelector)
    await (c1)[0].click();

    let c2 : any = await this.page.$x(authCodeContinueButtonSelector)
    await (c2)[0].click();
    ...
    do {
      ...
      let c : any = await this.page.$x(authCodeContinueButtonSelector)
      await (c)[0].click();
  ...
  public async parsePost(post: ElementHandle) {
    ...
    //HTMLElement
    const { authorName, content } = await this.page.evaluate(async (postElm: any, cssSelectors: typeof selectors): Promise<any> => {
        let postAuthorElm;
